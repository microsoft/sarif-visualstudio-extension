...
namespace Microsoft.Sarif.Viewer.CodeFinder.Internal.CStyle
{
    ...
    internal abstract class CStyleFinder : CodeFinderBase
    {
        ...
        public CStyleFinder(string fileContents)
            : base(fileContents)
        {
        }
        ...
        /// </param>
        /// <returns>A list of scope identifiers from the function signature.</returns>
        ...
        /// </summary>
        /// <param name="query">The query that we are using to search with.</param>
        /// <returns>The list of possible matches for the query.</returns>
        public override List<MatchResult> FindMatches(MatchQuery query)
        {
            // When looking for type class, use base finder
            if (query.TypeHint == MatchQuery.MatchTypeHint.Class)
            {
            ...
            foreach (FileSpan searchSpan in searchSpans)
            {
                ...
                do
                {
                    ...
                    if (pos != -1)
                    {
                        ...
                        matches.Add(new MatchResult(query.Id, new FileSpan(pos, pos + query.TextToFind.Length - 1), lineNumber, distanceFromLineHint, true, 0, IsStringLiteral(pos)));
        ...
        /// <summary>
        /// For the given query, returns all potential matches using the function signature.
        /// </summary>
        ...
        public override List<MatchResult> FindMatchesWithFunction(MatchQuery query)
        {
            ...
            if (string.IsNullOrWhiteSpace(query.FunctionSignature) &&
                (textToFind == "{" || textToFind == "}" || textToFind == "("))
            {
                return base.FindMatchesWithFunction(query);
            }
        ...
        /// </summary>
        /// <param name="startIndex">The index that the filespan must include.</param>
        ...
        /// </summary>
        /// <param name="line">The line number to find the scope span at.</param>
        /// <returns>The filespan at a particular line.</returns>
        ...
        /// <returns>List of scope identifying strings on or after the startindex.</returns>
        ...
        /// the first character of <paramref name="word"/>. When set to false, every character is checked.</param>
        /// <returns>True if the string is a valid C-Style word.</returns>
        ...
        /// </summary>
        /// <returns>A collection of spans to ignore.</returns>
